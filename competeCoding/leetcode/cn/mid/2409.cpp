...
class Solution {
 public:
  int countDaysTogether(string arriveAlice, string leaveAlice, string arriveBob,
    ...
    int ara = calc(arriveAlice, prefixSum);
    int lea = calc(leaveAlice, prefixSum);
    int arb = calc(arriveBob, prefixSum);
    int leb = calc(leaveBob, prefixSum);

    return max(0, min(lea, leb) - max(ara, arb) + 1);
  }

  int calc(string day, const vector<int>& prefixSum) {
